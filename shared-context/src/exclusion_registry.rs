...
/// Errors from the exclusion-registry loader. Defined locally so this module
/// is drop-in portable across bot crates.
#[derive(Debug, thiserror::Error)]
pub enum ExclusionError {
    #[error("failed to read exclusion registry at {path}: {source}")]
    Read {
        path: String,
        #[source]
        source: std::io::Error,
    },
    #[error("failed to parse exclusion registry: {0}")]
    Parse(String),
    #[error("invalid glob pattern {pattern:?}: {error}")]
    InvalidPattern { pattern: String, error: String },
    #[error("exclusion-registry deny [{axis:?}]: {reason}")]
    Denied { axis: DenyAxis, reason: String },
}
...
mod tests {
    ...
    fn clear_env() {
        // edition 2024 marks env::remove_var / set_var as unsafe. Here they are
        // guarded by ENV_LOCK so only one test at a time mutates process env.
        // SAFETY: single-threaded access under ENV_LOCK; no reads race.
        unsafe {
...
"#;
    ...
    fn kill_switch_blocks_everything() {
        let _g = ENV_LOCK.lock().unwrap();
        clear_env();
        // SAFETY: ENV_LOCK serialises env mutation across tests.
        unsafe {
        ...
        // SAFETY: same — serialised via ENV_LOCK.
        unsafe {
            std::env::remove_var("HYPATIA_AUTOMATION");
        }
        match d {
