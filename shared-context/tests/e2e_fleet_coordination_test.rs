...
use gitbot_shared_context::{BotId, Context, ContextStorage, Finding, ReportFormat, Severity};
...
fn e2e_single_bot_dispatch_process_collect() {
    ...
    ctx.start_bot(BotId::Rhodibot)
        .expect("start_bot must succeed for registered bot");
    ...
    assert!(
        readme_finding.is_some(),
        "RSR-MISSING-README must be present"
    );
...
fn e2e_multi_bot_dispatch_all_results_aggregated() {
    ...
    assert!(
        ctx.verifiers_complete(),
        "All verifiers must be complete before running finishers"
    );
    ...
    assert_eq!(
        ...
        "Summary must aggregate findings from all {} bots",
        total_bots
    );
    assert_eq!(
        summary.bots_run, total_bots,
        "bots_run must count all {} completing bots",
        total_bots
    );
    ...
    assert_eq!(
        ...
        "No error-severity findings were added"
    );
    assert_eq!(
        summary.total_warnings,
        verifiers.len(),
        "Verifiers each contributed one warning"
    );

    // No release blocks (no error-severity findings)
    assert!(
        !summary.blocks_release,
        "Warnings and info should not block release"
    );
}
...
fn e2e_bot_failure_does_not_prevent_other_bots() {
    let mut ctx = Context::new(
        ...
        PathBuf::from("/tmp/e2e-failure-isolation"),
    );
    ctx.register_all_bots();
    ...
    assert!(
        matches!(
            rhodibot_exec.status,
            gitbot_shared_context::bot::BotStatus::Failed
        ),
    ...
    assert_eq!(
        ctx.findings_from(BotId::Echidnabot).len(),
        1,
        "Echidnabot finding must be present"
    );
    assert_eq!(
        ctx.findings_from(BotId::Sustainabot).len(),
        1,
        "Sustainabot finding must be present"
    );

    // Rhodibot findings are empty (it failed before adding any)
    assert_eq!(
        ctx.findings_from(BotId::Rhodibot).len(),
        0,
        "Failed bot contributed no findings"
    );
    ...
    assert_eq!(
        ...
        "Only 2 findings from the successful bots"
    );
}
...
fn e2e_session_persistence_and_reload() {
    ...
    storage
        .save_context(&ctx)
        .expect("save_context must succeed");
    ...
    assert_eq!(
        loaded.repo_name, "persist-repo",
        "repo_name must survive round-trip"
    );
    assert_eq!(
        loaded.findings.len(),
        expected_findings,
    ...
    assert_eq!(
        ...
        "session_id must be identical after reload"
    );
...
fn e2e_report_generation_pipeline() {
    ...
    ctx.add_finding(
        Finding::new(
            ...
            "RSR-001",
            Severity::Error,
            "Missing SPDX headers on 3 files",
        )
    ...
    let parsed: serde_json::Value =
        serde_json::from_str(&json_report).expect("JSON report must be valid JSON");
    assert!(
...
fn e2e_findings_severity_pipeline_release_gate() {
    ...
    // Case 2: Only warnings → no block
    {
        ...
        assert!(
            !ctx.blocks_release(),
            "Warnings alone must not block release"
        );
    }
    ...
    // Case 3: Error present → blocks release
    {
        ...
        assert!(
            ctx.has_errors(),
            "has_errors must return true when Error findings present"
        );
    }
    ...
    // (demonstrates the finding pipeline supports fixability tracking)
    {
        ...
        assert_eq!(
            ctx.findings.fixable().len(),
            1,
            "One fixable finding must be tracked"
        );
